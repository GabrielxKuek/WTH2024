...
void loop() {
  ...
  if (GameMode == 0) writeToDisplaysLineByLine("OPTION 1","> Play Game <","","OPTION 2","CHECK STATS", "" );
  else writeToDisplaysLineByLine("OPTION 1","Play Game","","OPTION 2","> CHECK STATS <", "" );
  //GameMode = 0  is Play. GameMode = 1  is Stats.

  if (digitalRead(18)==1) {
    delay(50);
    int Menu2 = 1;
    while (Menu2 == 1) {
    if (GameMode == 0) writeToDisplaysLineByLine("GAME STARTS","> FIGHT <","","","","");
    else writeToDisplaysLineByLine("SHOW STATS","SHOW NUMBERS","","","","");
    if (digitalRead(17)==1) {Menu2 = 0; GameMode = 0;};
    };
  };
